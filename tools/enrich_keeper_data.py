...
Outputs in the newest per-run folder for the supplied league:
  - keeper_board.json  Humanized roster-by-roster keeper candidates with prior
                       draft round, acquisition path, keeper costs, and 2026
                       keeper-set constraints.
  - traded_picks.json  Current Sleeper traded-pick records for the league.

2026 keeper rules encoded for the keeper league:
  - Round 1 players are not keepable.
  - Any player drafted in the prior season retains that draft round as the
    keeper base round, even if later dropped, re-added, waived, or traded.
  - A player who was not drafted has a Round 14 keeper base round.
  - Round collisions cascade earlier: if a keeper's base round is occupied,
    the additional keeper moves to 13, then 12, etc. until an open round exists.
  - A seven-keeper set may include at most one keeper assigned to Round 2 or
    Round 3. If it includes one, the other six keepers must be Round 4 or later.
    If it includes neither, all seven keepers may come from the remaining
    eligible rounds.
"""
...
USER_AGENT = "sleeper-sync/1.4 (keeper-enrichment)"
UNDRAFTED_BASE_ROUND = 14
EARLY_KEEPER_ROUNDS = {2, 3}


def _omit_player_nicknames(data):
    ...
    for attempt in range(1, retry + 1):
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
                return json.loads(resp.read().decode("utf-8"), object_hook=_omit_player_nicknames)
...
def fetch_transactions(
    league_id: str, weeks: Iterable[int] = range(1, 19)
) -> List[Dict[str, Any]]:
...
def latest_acquisition(
    ...
    """Find the latest add of player_id to this manager's roster."""
...
def load_or_fetch_previous_history(
    ...
    if all(
        path.exists()
        for path in (league_path, rosters_path, picks_path, txns_path)
    ):
        return (
    ...
    league_path.write_text(
        json.dumps(previous_league, indent=2), encoding="utf-8"
    )
    rosters_path.write_text(
        json.dumps(previous_rosters, indent=2), encoding="utf-8"
    )
    picks_path.write_text(
        json.dumps(previous_picks, indent=2), encoding="utf-8"
    )
    txns_path.write_text(
        json.dumps(previous_transactions, indent=2), encoding="utf-8"
    )
    return (
        previous_league,
        previous_rosters,
    ...
    try:
        return (
            dt.datetime.fromtimestamp(
                int(value) / 1000, tz=dt.timezone.utc
            )
            .isoformat()
            .replace("+00:00", "Z")
        )
...
def submitted_rule_check(
    player_rows: List[Dict[str, Any]], max_keepers: int
) -> Dict[str, Any]:
    submitted_rows = [p for p in player_rows if p.get("keeper_submitted")]
    assigned_rounds = [
        int(p["keeper_submitted_round"])
        for p in submitted_rows
        if p.get("keeper_submitted_round") is not None
    ]
    early_count = sum(r in EARLY_KEEPER_ROUNDS for r in assigned_rounds)
    has_round_one = any(r == 1 for r in assigned_rounds)

    valid_so_far = not has_round_one and early_count <= 1
    reason = None
    if has_round_one:
        reason = "round_1_not_keepable"
    elif early_count > 1:
        reason = "more_than_one_round_2_or_3_keeper"

    return {
        "submitted_keeper_count": len(submitted_rows),
        "max_keepers": max_keepers,
        "assigned_rounds": sorted(assigned_rounds),
        "round_2_or_3_keeper_count": early_count,
        "valid_so_far": valid_so_far,
        "invalid_reason": reason,
        "complete": len(submitted_rows) == max_keepers if max_keepers else False,
    }


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Enrich a SleeperAgent run with keeper costs and traded picks."
    )
    parser.add_argument("--league", required=True, help="Current Sleeper league ID")
    parser.add_argument(
        "--out", default="./docs", help="SleeperAgent docs/output directory"
    )
    ...
    (run_dir / "traded_picks.json").write_text(
        json.dumps(traded_picks, indent=2), encoding="utf-8"
    )
    ...
    current_transactions = (
        load_run_transactions(run_dir, league_id) if keeper_enabled else []
    )
    ...
    for roster in sorted(
        rosters or [], key=lambda r: int(r.get("roster_id") or 0)
    ):
        ...
        for raw_pid in roster.get("players") or []:
            ...
            acquisition = latest_acquisition(
                pid, roster_targets, all_transactions
            )
            ...
            if acquisition:
                ...
                or (
                    previous_roster_id is not None
                    and int(prior.get("roster_id") or -1)
                    == previous_roster_id
                )
            ):
                acquired_via = "draft"
            elif prior:
                acquired_via = "trade_or_other"
            else:
                acquired_via = "undrafted"

            original_round = int(prior["round"]) if prior else None
            submitted_round = (
                submitted_keeper.get("round") if submitted_keeper else None
            )
            ...
            if not keeper_enabled:
                ...
                eligibility_reason = (
                    "prior_draft_round_available"
                    if original_round is not None
                    else "undrafted_round_14_rule"
                )
                base_round = (
                    original_round
                    if original_round is not None
                    else UNDRAFTED_BASE_ROUND
                )
                if submitted_round is not None:
                    keeper_cost_round = submitted_round
                    cost_source = "current_submitted_keeper"
                else:
                    keeper_cost_round = base_round
                    cost_source = (
                        "previous_draft"
                        if original_round is not None
                        else "undrafted_round_14_rule"
            ...
            player_rows.append(
                {
                    ...
                    "original_draft_pick_no": (
                        prior.get("pick_no") if prior else None
                    ),
                    "original_draft_slot": (
                        prior.get("draft_slot") if prior else None
                    ),
                    "original_drafted_by": (
                        prior.get("picked_by") if prior else None
                    ),
                    "original_draft_roster_id": (
                        prior.get("roster_id") if prior else None
                    ),
                    "acquired_via": acquired_via,
                    "acquisition_transaction_id": (
                        acquisition.get("transaction_id")
                        if acquisition
                        else None
                    ),
                    "acquisition_time": (
                        iso_from_ms(acquisition.get("created"))
                        if acquisition
                        else None
                    ),
                    "acquisition_league_id": (
                        acquisition.get("league_id")
                        if acquisition
                        else None
                    ),
                    ...
                    "keeper_base_round": base_round,
                    "keeper_cost_round": keeper_cost_round,
                    "keeper_cost_source": cost_source,
                    "keeper_round_may_move_for_collision": (
                        bool(keeper_eligible) and submitted_round is None
                    ),
                    "current_draft_round": current_draft.get(pid, {}).get(
                        "round"
                    ),
                }
            )
        ...
        teams.append(
            {
                ...
                "team_name": team_name,
                "submitted_keeper_rule_check": submitted_rule_check(
                    player_rows, max_keepers
                ),
                "players": player_rows,
            }
        )
    ...
    payload = {
        ...
        "rules": {
            ...
            "drafted_player_base_cost": "prior/original draft round, regardless of later drop, waiver claim, re-add, or trade",
            "undrafted_player_base_cost_round": UNDRAFTED_BASE_ROUND,
            "round_collision_rule": "if a keeper round is occupied, move the additional keeper one round earlier repeatedly until an open round is found",
            "round_2_or_3_constraint": {
                "max_keepers_from_rounds_2_and_3_combined": 1,
                "if_used": "the other six keepers must be Round 4 or later",
                "if_not_used": "all seven keepers may come from the remaining eligible rounds",
            },
        },
        "teams": teams,
    }
    (run_dir / "keeper_board.json").write_text(
